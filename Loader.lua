...
local function LoadSetting()
	...
	if game.Players.LocalPlayer.Team ~= nil then
		...
		--spawn(function()
			...
			if _G.Setting_table.C then
				_G.Setting_table.C = _G.Setting_table.C 
			else
				_G.Setting_table.C = 0
			end

			if _G.Setting_table.V then
				_G.Setting_table.V = _G.Setting_table.V 
			else
				_G.Setting_table.V = 30
			end

			if _G.Setting_table.B then
				_G.Setting_table.B = _G.Setting_table.B 
			else
				_G.Setting_table.B = 0
			end
		--end)

		local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/zeroscgaming/lib-zpshub/main/PepsiUi",true))()
		local Win = library:CreateWindow("baor HUB | XZX TEAM Version 1.10.0",Enum.KeyCode.RightControl)
		
		local H = Win:CreateTab("Home")
		...
		local HZ = H:CreateSector("Home","Left")
		...
		local IslandTP = Island:CreateSector("Island Teleport Bypass","Right")
		local IslandTeleport = Island:CreateSector("Teleport","Right")
		...
		local MiscStatus = Status:CreateSector("Misc","Right")
		...
		HZ:AddLabel("Soucre Cfframe Hub")
		HZ:AddLabel("By XZX TEAM")
		HZ:AddLabel("Discord Warning")
		HZ:AddSeperator("Update [+] Delete [-] Eror [!]")
		HZ:AddLabel("[+] Auto raid Pirate")
		HZ:AddLabel("[+] Config Admin")
		HZ:AddLabel("[+] HOME")
		HZ:AddLabel("[+] Update script 13/3/2023")
		HZ:AddLabel("[!] Auto Elite")
		HZ:AddLabel("[+] Teleport")
		HZ:AddLabel("[+] Pos")
		HZ:AddLabel("[+] FIX LAG ANTI BAN")
		HZ:AddLabel("[+] Status Check Elite Spawn")
		HZ:AddLabel("[+] Status Check FULL MON")
		HZ:AddLabel("[+] Status Check Mirage island")
...
AutoFarm:AddToggle("Auto Raid Pirate",_G.Setting_table.Auto_Quest_Tushita_2,function(vu)
...
spawn(function()
	while wait() do
		...
		CustomMethod:AddSlider("Position C",-40,_G.Setting_table.X,100,1,function(t)
			_G.Setting_table.C = t
				SaveSetting()
		end)
		CustomMethod:AddSlider("Position V",-40,_G.Setting_table.Y,100,1,function(t)
			_G.Setting_table.V = t
				SaveSetting()
		end)
		CustomMethod:AddSlider("Position Z",-40,_G.Setting_table.Z,100,1,function(t)
			_G.Setting_table.B = t
				SaveSetting()
		end)
	...
	elseif game.PlaceId == 7449423635 then
		...
		BuyWeapon:AddToggle("Auto Buy Legendary Sword",false,function()
			game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer","1")
			game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer","2")
			game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("LegendarySwordDealer","3")
		end)
		...
		local Miscmon =  MiscStatus:AddLabel("HX")

local FullMoon = MiscStatus:AddLabel("Check Moon")
        task.spawn(function()
            while task.wait() do
                pcall(function()
		...
		local mr =  MiscStatus:AddLabel("HX")
...
task.spawn(function()
	while task.wait() do
		...
		local Elite_Hunter_Status =  MiscStatus:AddLabel("HX")
						...
						if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false then
							if _G.Setting_table.AutoElite then
								if game.ReplicatedStorage:FindFirstChild("Diablo [Lv. 1750]") or game.ReplicatedStorage:FindFirstChild("Urban [Lv. 1750]") or game.ReplicatedStorage:FindFirstChild("Deandre [Lv. 1750]") then
									...
									BTP(CFrame.new(-5419.79296875, 313.7849426269531, -2823.969482421875))
				...
				if _G.Setting_table.AutoFarmBone then
					pcall(function()
						if game.Workspace.Enemies:FindFirstChild("Reborn Skeleton [Lv. 1975]") or game.Workspace.Enemies:FindFirstChild("Living Zombie [Lv. 2000]") or game.Workspace.Enemies:FindFirstChild("Demonic Soul [Lv. 2025]") or game.Workspace.Enemies:FindFirstChild("Posessed Mummy [Lv. 2050]") then
							...
							Tween(CFrame.new(-9467.408203125, 172.1048126220703, 6191.10986328125))
							Tween(CFrame.new(-9468.8779296875, 172.1048126220703, 6120.71435546875))
							Tween(CFrame.new(-9543.8232421875, 172.1048126220703, 6130.02978515625))
							Tween(CFrame.new(-9536.001953125, 172.1048126220703, 6192.95654296875))
						end
					end)
				end
			end
									...
									if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
										repeat task.wait()
											while task.wait() do
												...
												Tween(v2.HumanoidRootPart.CFrame * CFrame.new(_G.Setting_table.X,_G.Setting_table.Y,_G.Setting_table.Z))
                                                                                		wait(0.2)
												Tween(v2.HumanoidRootPart.CFrame * CFrame.new(_G.Setting_table.C,_G.Setting_table.V,_G.Setting_table.B))
												sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
											end
	...
	function SendHook(LinkHook)
		local Embed = {
			 ["username"] = "baor HUB Webhook",
			 ["avatar_url"] = "https://media.discordapp.net/attachments/909095824972218409/1039041801685839932/19_20221101155849.png",
			["embeds"] = {
				{
					["title"] = "**Blox Fruit // baor Hub Webhook**",
					["color"] = tonumber(0xff0000),
					["type"] = "rich",
					["fields"] =  {
						{
