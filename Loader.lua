...
local win = Flux:Window("Royx Hub", "Ro Ghoul", Color3.fromRGB(255, 70, 70 ), Enum.KeyCode.RightControl)
...
local NameMon = {"Human","Aogiri Member","Investigator"}
...
StageFarm = ""
page1:Dropdown('Select Stage',StatseSelects,function(vu)
    StageFarm = vu
end)
...
page1:Toggle('Auto Farm',false,function(vu)
    Auto_Farm = vu
    if Auto_Farm and StageFarm == "" then
        DiscordLib:Notification("Auto Farm", "Select State First", "OKAY")
    elseif Auto_Farm and MonsterFarm == "" then
        DiscordLib:Notification("Auto Farm", "Select Monster First", "OKAY")
    else
        _G.AutoFarm = vu
    end
end)
...
Speed = 150
page1:Slider('Speed [Defualt 150]', 0,200,Speed,function(vu)
    Speed = vu
end)
...
spawn(function()
    while wait() do
        pcall(function()
            if Physical then
                Statsremote("PhysicalAddButton",1)
            end
            if Quinque_Kagune then
                Statsremote("WeaponAddButton",1)
            end
            if Durability then
                Statsremote("DurabilityAddButton",1)
            end
            if SpeedUp then
                Statsremote("SpeedAddButton",1)
            end
        end)
    end
end)
